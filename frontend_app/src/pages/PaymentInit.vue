...
onUnmounted(() => {                    // чистим таймеры при уходе
...
function pollStatus(sessionId: string, key: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    pollIv = setInterval(async () => {
      try {
        const res = await fetch(`http://localhost:8000/payment/status/${sessionId}`, {
...
<template>
  <div class="flex flex-col items-center min-h-screen bg-gradient-to-b from-sky-50 to-white px-4 py-8">
    <main class="flex flex-col items-center justify-center text-center space-y-8 max-w-xl flex-1 w-full">
      ...
      <h2 class="text-2xl font-semibold text-blue-600">{{ price.toLocaleString() }} ₽</h2>
      <img src="/sbp.png" alt="Ingosstrakh logo" class="w-100 h-auto my-6 select-none pointer-events-none self-start" />
      <div class="w-full grid gap-4 grid-cols-1 sm:grid-cols-2">
        <!-- Кредитная карта -->
        <button @click="payByCard" :disable="loading"
          class="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 p-6 shadow-sm transition-all w-full"
          :class="loading ? 'border-blue-500 bg-blue-50 opacity-60 cursor-wait' : 'border-gray-300 bg-white hover:bg-gray-50'">
